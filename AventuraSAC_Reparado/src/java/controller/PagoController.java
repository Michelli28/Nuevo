...
public class PagoController {
    ...
    public ModelAndView NuevoPago1(Model model, HttpServletRequest request) {
        ...
        double acu = p.getSaldo() - p.getAcumulado();
        mv.addObject("p", acu);
    ...
    public ModelAndView NuevoPago1(HttpServletRequest request) throws Exception {
         ...
         */
            if(p.getMonto() >= (pedido.getSaldo() - pedido.getAcumulado()) && (p.getMonto() + pedido.getAcumulado()) < pedido.getSaldo() || p.getMonto() <= (pedido.getSaldo() - pedido.getAcumulado()) && (p.getMonto() + pedido.getAcumulado()) < pedido.getSaldo()) {
                pedido.setIdEstado(repo2.findEstadopedido(Estado));
                repo1.edit(pedido);
            } 
            double ss= pedido.getAcumulado();
            if( ss /  pedido.getSaldo() == 1) {
